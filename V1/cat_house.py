...
class KediEvi:
    ...
    def handle_event(self, event, kedi, top_alindi):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 3 and self.rect.collidepoint(event.pos):
                ...
                if self.menu_acik:
                    log_message("Ev menüsü açıldı.") # LOGLAMA EKLENDİ
                ...
                if event.button == 1:
                    for buton_adi, buton_rect in self.menu_butonlari.items():
                        if buton_rect.collidepoint(event.pos):
                            if buton_adi == "top_al":
                                self.menu_acik = False
                                log_message("Top al butonuna basıldı.") # LOGLAMA EKLENDİ
                                return True  # Top alındı, True döndür
            ...
            if self.dragging:
                ...
                elif kedi.evin_icinde_mi(self.rect): # Bu satır eklendi
                    kedi.istatistikler.uyu() # Bu satır eklendi
    ...
    def hareket_ettir(self, kedi):
        if self.dragging:
            ...
            self.y = max(0, min(self.y, self.screen_height - self.rect.height))
            self.rect.topleft = (self.x, self.y)
            self.menu_acik = False  # Menüyü kapat